...
- **Host build for tests:** the `buildXdeltaHost` Gradle task compiles a Linux x86_64 copy of the decoder + CLI under `build/xdeltaHost` (never committed) so the JVM unit test `XdeltaApplierTest` can exercise `XdeltaApplier` end-to-end via the `hylianbox.xdelta.jni.path` / `hylianbox.xdelta.cli.path` system properties, without the Android toolchain.
