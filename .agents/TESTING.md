...
Equipped-item coverage verifies the exact OoT/MM C-button and current equipment offsets, all supported byte lanes, MM form-independent C assignments, sword/shield item-id conversion, and equipment polling while progress auto-tracking is disabled. `EquippedItemIconMapTest` verifies the complete locally extracted HUD-icon ranges and RAW/YAR archive offsets. `ItemAmmoResolverTest` verifies the per-button ammo mapping (OoT/MM slots, magic-arrow-to-bow sharing, MM pictograph quest flag, null for quantity-less items), parser ammo exposure per C button, and ammo-only changes marking equipment dirty so the badge refreshes. The mapped-area/minimal overlay is explicitly excluded from icon extraction/rendering.

ROM-hack compatibility coverage relocates otherwise vanilla-layout OoT/MM `SaveContext` fixtures across all supported byte lanes. It also verifies that an invalid signature decoy is rejected, a valid signature observed before gameplay is retried after game mode becomes ready, and the poller emits equipped-button state from the relocated address. These synthetic cases do not claim support for hacks that change the structure itself.
