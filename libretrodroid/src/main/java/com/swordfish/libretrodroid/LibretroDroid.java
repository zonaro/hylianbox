...
public class LibretroDroid {
    ...
    /**
     * Returns a direct ByteBuffer aliasing the requested core memory region,
     * or null when no game is loaded / the region is unavailable. The buffer
     * aliases memory owned by the loaded core: it is only valid while the
     * current game stays loaded and must not be cached across unload/reload.
     */
