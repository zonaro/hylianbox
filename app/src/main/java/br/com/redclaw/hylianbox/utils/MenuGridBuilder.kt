 ...
 * @param tintIcon when true (default) the icon is tinted to the menu's on-surface-variant color;
 * set false for self-colored drawables (e.g. the Auto-Ocarina icon) that must keep their own
 * palette.
 * @param isEnabled returns whether the cell is currently interactive. Disabled cells are greyed
 * (alpha 0.38f) and excluded from click / focus navigation, so a D-pad naturally skips them.
 * Defaults to always enabled; the in-game menu uses this to grey out RetroAchievements items until
 * the RA session resolves for the running game.
...
data class MenuSection(@StringRes val titleRes: Int, val items: List<MenuActionItem>)
...
/**
 * Live references to a non-toggle cell, used to refresh its enabled / greyed state (e.g.
 * RetroAchievements items whose availability resolves after the menu is first built). Toggles are
 * excluded to avoid double management.
 */
...
/**
 * Builds the shared grid-menu view used by both the in-game menu and the Library per-game context
 * menu. The layout mirrors the in-game menu exactly: a scrollable [android.widget.ScrollView]
 * (dialog_game_menu.xml) whose vertical [LinearLayout] holds a bold header per [MenuSection]
 * followed by its icon+label cells laid out in rows of [colCount] via nested weighted
 * [LinearLayout]s (so every cell measures to an equal width inside the unbounded ScrollView).
 *
 * Physical-controller key badges are shown only when a physical controller is connected, i.e. when
 * [GamePad.shouldShowGamePads] is false.
 */
object MenuGridBuilder {
     ...
     * Build the menu view.
     *
     * @param context used for inflation and resource lookups (must be an Activity so the
     * physical-controller badge rule can be evaluated).
     * @param sections category sections (headers + items) in display order.
     * @param onItemActivated invoked when a cell is tapped; the caller wires dismissal /
     * post-action behaviour here, keeping one activation path for both taps and physical keys.
     */
    fun build(
            context: Context,
        ...
        val headerTopMargin =
                context.resources.getDimensionPixelSize(R.dimen.menu_header_top_margin)
        val headerBottomMargin =
                context.resources.getDimensionPixelSize(R.dimen.menu_header_bottom_margin)
        ...
        for (section in sections) {
            ...
            container.addView(
                    ...
                    LinearLayout.LayoutParams(
                            ...
                            .apply { setMargins(0, headerTopMargin, 0, headerBottomMargin) }
            )
            ...
            while (index < items.size) {
                ...
                for (j in index until end) {
                    ...
                    /* Track every cell for enabled / greyed refresh so toggles
                    (e.g. fast-forward blocked by RetroAchievements) can also
                    be greyed live. Default isEnabled is true, so no effect
                    for items that stay always enabled. */
