...
import br.com.redclaw.hylianbox.databinding.ActivityRaProfileBinding
import br.com.redclaw.hylianbox.retroachievements.data.RaLastGame
import br.com.redclaw.hylianbox.retroachievements.data.RaRecentAchievement
import br.com.redclaw.hylianbox.retroachievements.data.RaRecentlyPlayed
 ...
 *
 * Mirrors the Vigia-ai dashboard: header (avatar, motto, status, member since, rich presence),
...
class RaProfileActivity : ScaledAppCompatActivity() {
        ...
        binding.raProfileOpenWeb.setOnClickListener {
            ...
            if (!username.isNullOrBlank()) {
                startActivity(
                        Intent(
                                Intent.ACTION_VIEW,
                                Uri.parse("https://retroachievements.org/user/$username")
                        )
                )
            }
        }
    ...
    private fun render(state: RaProfileUiState) {
        ...
        binding.raProfileMessage.isVisible =
                state is RaProfileUiState.SignedOut ||
                        state is RaProfileUiState.NeedsApiKey ||
                        state is RaProfileUiState.Error
        binding.raProfileRetry.isVisible = state is RaProfileUiState.Error
        when (state) {
            is RaProfileUiState.Content -> bindProfile(state.profile)
            RaProfileUiState.SignedOut ->
                    binding.raProfileMessage.setText(R.string.ra_profile_signed_out)
            RaProfileUiState.NeedsApiKey ->
                    binding.raProfileMessage.setText(R.string.ra_profile_needs_api_key)
            RaProfileUiState.Error -> binding.raProfileMessage.setText(R.string.ra_profile_error)
            RaProfileUiState.Loading -> Unit
        }
    }

    private fun bindProfile(profile: RaUserProfile) =
            with(binding) {
                ...
                bindOptional(
                        ...
                        profile.memberSince?.let { getString(R.string.ra_profile_member_since, it) }
                )
    ...
    private fun bindMetrics(profile: RaUserProfile) =
            with(binding) {
                raProfileMetrics.removeAllViews()

                val rankSub =
                        if (profile.rank != null && profile.totalRanked != null) {
                            getString(
                                    R.string.ra_profile_metric_rank_sub,
                                    formatNumber(profile.rank),
                                    formatNumber(profile.totalRanked)
                            )
                        } else {
                            null
                        }
                addMetricRow(
                        ...
                        profile.totalSoftcorePoints?.let {
                            getString(R.string.ra_profile_metric_softcore_sub, formatNumber(it))
                        }
                )
                ...
                addMetricRow(
                        ...
                        completion?.let {
                            getString(R.string.ra_profile_metric_completion_sub, formatNumber(it))
                        },
                        getString(R.string.ra_profile_metric_beaten),
                        formatNumber(beatenHardcore),
                        beatenSoftcore?.let {
                            getString(
                                    R.string.ra_profile_metric_beaten_softcore_sub,
                                    formatNumber(it)
                            )
                        }
                )
    ...
    private fun addMetricRow(
        ...
        row.addView(
                metricCard(label2, value2, sub2).apply {
    ...
    private fun metricCard(label: String, value: String, sub: String?): View {
        val card =
                layoutInflater.inflate(
                        R.layout.item_ra_profile_metric,
                        binding.raProfileMetrics,
                        false
                )
    ...
    private fun bindLastGame(lastGame: RaLastGame?) =
            with(binding) {
    ...
    private fun bindRecentlyPlayed(games: List<RaRecentlyPlayed>) =
            with(binding) {
    ...
    private fun gameRow(game: RaRecentlyPlayed): View {
        val row =
                layoutInflater.inflate(
                        R.layout.item_ra_profile_game,
                        binding.raProfileRecentGames,
                        false
                )
        row.findViewById<TextView>(R.id.ra_profile_game_title).text =
                game.title?.takeIf { it.isNotBlank() }
                        ?: getString(R.string.ra_profile_unknown_game)
        ...
        val progress = buildString {
            if (game.numAchieved != null && game.achievementsTotal != null) {
                append(
                        getString(
                                R.string.ra_profile_game_progress,
                                game.numAchieved,
        ...
        row.findViewById<android.widget.ImageView>(R.id.ra_profile_game_image).load(
                        game.imageIcon
                ) {
            crossfade(true)
            placeholder(R.drawable.ic_hack)
    ...
    private fun bindRecentAchievements(achievements: List<RaRecentAchievement>) =
            with(binding) {
    ...
    private fun achievementRow(ach: RaRecentAchievement): View {
        val row =
                layoutInflater.inflate(
                        R.layout.item_ra_profile_achievement,
                        binding.raProfileRecentAchievements,
                        false
                )
        row.findViewById<TextView>(R.id.ra_profile_achievement_title).text =
                ach.title?.takeIf { it.isNotBlank() }
                        ?: getString(R.string.ra_profile_unknown_achievement)
        bindOptional(row.findViewById(R.id.ra_profile_achievement_game), ach.gameTitle)
        val meta = buildString {
            if (ach.points != null)
                    append(getString(R.string.ra_profile_achievement_points, ach.points))
            if (ach.hardcore != null) {
                if (isNotEmpty()) append(" · ")
                append(
                        getString(
                                if (ach.hardcore) R.string.ra_profile_achievement_hardcore
                                else R.string.ra_profile_achievement_softcore
                        )
                )
            }
        ...
        row.findViewById<android.widget.ImageView>(R.id.ra_profile_achievement_badge).load(
                        ach.badgeUrl
                ) {
            crossfade(true)
            placeholder(R.drawable.ic_trophy)
