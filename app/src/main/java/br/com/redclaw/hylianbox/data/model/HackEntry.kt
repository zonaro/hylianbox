package br.com.redclaw.hylianbox.data.model
        ...
        companion object {
                fun fromJson(o: JSONObject): ChangelogEntry =
                        ChangelogEntry(
                                ...
                                content =
                                        if (o.isNull("content")) null
                                        else o.optString("content", null)
                        )
        }
}
        ...
        companion object {
                fun fromJson(o: JSONObject): CatalogImportSource =
                        CatalogImportSource(
                                ...
                                modUrl =
                                        if (o.has("modUrl") && !o.isNull("modUrl"))
                                                o.getString("modUrl")
                                        else null
                        )
        }
...
data class DeveloperLink(val label: String, val url: String) {
        ...
        companion object {
                fun fromJson(o: JSONObject): DeveloperLink =
                        DeveloperLink(
...
data class HackEntry(
        ...
        /**
         * Stable, store-agnostic identity for cross-catalog dedupe. Computed (not stored) by
         * delegating to [CanonicalIdResolver]; two catalog entries that describe the same hack
         * resolve to the same canonical id regardless of the store that published them.
         */
        ...
        companion object {
                fun fromJson(o: JSONObject): HackEntry =
                        HackEntry(
                                ...
                                retroAchievements =
                                        if (o.has("retroAchievements") &&
                                                        !o.isNull("retroAchievements")
                                        ) {
                                                runCatching {
                                ...
                                videos =
                                        if (o.has("videos"))
                                                jsonToStringList(o.getJSONArray("videos"))
                                        else emptyList(),
                                developerLinks =
                                ...
                                downloadTarget =
                                        if (o.has("downloadTarget") && !o.isNull("downloadTarget")
                                                ...
                                                parseDownloadTarget(
                                                        o.getJSONObject("downloadTarget")
                                                )
                                        } else {
                                                null
                                        }
                                        ...
                                        when (o.optString("type")) {
                                                "direct" ->
                                                        DownloadTarget.DirectPatch(
                                                                PatchRef.fromJson(
                                                                        o.getJSONObject("patch")
                                                                )
                                                        )
                                                "github" ->
                                                        DownloadTarget.GitHubRelease(
                                                                o.getString("repoUrl")
                                                        )
                                                "external" ->
                                                        DownloadTarget.ExternalLink(
                                                                o.getString("url")
                                                        )
                                                else -> null
                                        }
                 ...
                 *
                 * Primary: matching [canonicalId] (covers slug normalization + alias map).
                 * Fallback: both carry non-null patch [Checksums] whose CRC32, MD5, and SHA-1 all
                 * match. A canonical match with differing checksums still counts as the same hack
