...
class RaCredentialStore(private val context: Context) {
     ...
     *
     * This is the key from https://retroachievements.org/settings?tab=applications used as the `y`
     * query parameter of the Web API. It is distinct from the rcheevos login token ([getToken]) and
     * is required by the profile screen. Treat as secret: never log or display.
     */
    ...
    /**
     * Encrypted durable award journal, without login tokens; survives logout for account isolation.
     */
    internal fun readPendingAwards(): String = prefs.getString("ra_pending_awards_v1", "[]") ?: "[]"

    /** Synchronous commit ensures the award exists on disk before its HTTP submission. */
    internal fun writePendingAwards(json: String) {
        check(prefs.edit().putString("ra_pending_awards_v1", json).commit()) {
            "Unable to persist pending RetroAchievements awards"
        }
    }
