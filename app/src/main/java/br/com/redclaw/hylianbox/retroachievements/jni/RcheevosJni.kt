 ...
 *
 * Implementations must be cheap and must not block: heavy work belongs in coroutines dispatched by
 * the caller. All callbacks carry small JSON payloads parsed with org.json on the Kotlin side.
 */
interface RaNativeListener {

    /**
     * rcheevos needs an HTTP round trip. [requestId] correlates the response; [postData] is null
     * for GET requests. Execute via OkHttp off the calling thread, then call
     * [RcheevosJni.nativeCompleteServerRequest].
     */
    fun onServerRequest(requestId: Int, url: String, postData: String?)

    /**
     * An async operation (login, identify/load game) finished. [resultCode] is an RC_ error code
     * (RC_OK == 0 on success); [errorMessage] is a server/native message or null.
     */
    fun onAsyncResult(opId: Int, resultCode: Int, errorMessage: String?)

    /**
     * A client event fired (achievement unlocked, challenge indicator, leaderboard start/submit,
     * server error). [eventType] matches the RC_CLIENT_EVENT_* values; [payloadJson] carries event
     * details.
     */
 ...
 *
 * Owns the rc_client lifecycle: creation binds a single [RaNativeListener]; HTTP stays in Kotlin
 * (see [RaNativeListener.onServerRequest]); emulated memory is aliased through
 * [nativeSetMemoryRegion] once per game load.
 *
 * Threading: session calls (doFrame, login, loadGame) are main-thread entry points;
 * [nativeCompleteServerRequest] may be called from any OkHttp worker thread. Structured getters
 * return small JSON documents.
 */
object RcheevosJni {
    ...
    /** Returns the rcheevos User-Agent clause (e.g. "rcheevos/12.4.0"). */
    external fun getUserAgentClause(): String
    ...
    /**
     * Hashes [filePath] with rhash and identifies it against the RA database, loading achievements
     * when matched. Result via onAsyncResult(opId, ...).
     */
    ...
    /**
     * Aliases emulated memory for rcheevos reads. Pass null to detach. The buffer MUST be a direct
     * ByteBuffer obtained from GLRetroView.getMemoryRegion and must remain valid until replaced.
     */
     ...
     * failure. Body bytes must be UTF-8 encoded.
     */
    ...
    /**
     * All achievements grouped by bucket as a JSON array; empty array when no game is loaded.
     * Buckets follow lock-state grouping.
     */
    external fun nativeGetAchievementListJson(): String

    /**
     * Builds the standalone resolve-hash request for [gameHash]. Returns [url, postData] where
     * postData may be null (GET), or null on failure. Used at install time without a live session.
     */
    ...
    /**
     * Computes the RetroAchievements hash of [filePath] using rhash with the N64 console id.
     * Returns "" on failure. Blocking file I/O: call off the main thread.
     */
    external fun nativeComputeRomHash(filePath: String): String

    /**
     * Builds the fetch-game-data request for [gameId]. [username]/[apiToken] authenticate the
     * request. Returns [url, postData] or null.
     */
     ...
     * `{id,title,image_url,achievements:[...],leaderboards:[...]}`, or "null" on failure. Blocking
     * parse of a potentially large body: call off the main thread.
     */
    external fun nativeProcessFetchGameDataResponse(responseBody: String): String

    /**
     * Builds the fetch-user-unlocks request for [gameId]. Credentials are required by the server.
     * Returns [url, postData] or null.
     */
