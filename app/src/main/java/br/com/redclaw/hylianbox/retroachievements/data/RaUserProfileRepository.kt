...
package br.com.redclaw.hylianbox.retroachievements.data
 ...
 *
 * Mirrors the fields surfaced by the Vigia-ai dashboard: the user summary (API_GetUserSummary)
 * merged with award counters (API_GetUserAwards) and completion progress
 * (API_GetUserCompletionProgress). No credential-like field is ever stored or exposed.
 */
 ...
 *
 * The profile uses the public Web API, which authenticates with the Web API key from
 * https://retroachievements.org/settings?tab=applications (the `y` query parameter) — not the
 * rcheevos login token. The key is supplied only as an encoded query parameter and is never logged
 * or cached with the profile.
 */
class RaUserProfileRepository(
    ...
    /**
     * Gets a profile, using a short-lived cache unless [forceRefresh] is true. Use [refreshProfile]
     * for an explicit user-requested refresh.
     */
    ...
    private suspend fun fetchAndCache(username: String, apiKey: String): Result<RaUserProfile> {
        val summary =
                fetchJson(SUMMARY_ENDPOINT, username, apiKey, "g=3&a=5")
        ...
        val profile =
                parseProfile(summary, awards, completion, username)
    ...
    private fun readCached(username: String): CachedProfile? =
            runCatching {
                        ...
                        val profile =
                                root.optJSONObject(CACHE_PROFILE)?.toProfile(username)
                                        ?: return null
    ...
    private fun parseProfile(
        ...
        val username =
                firstString(summary, "User", "user", "Username", "username")?.takeIf {
                    it.isNotBlank()
                }
                        ?: fallbackUsername
        val avatar =
        ...
        val memberSince =
                firstString(summary, "MemberSince", "memberSince")?.takeIf { it.isNotBlank() }
        val status = firstString(summary, "Status", "status")?.takeIf { it.isNotBlank() }
        val richPresence =
                firstString(summary, "RichPresenceMsg", "richPresenceMsg")?.takeIf {
                    it.isNotBlank()
                }
        ...
        val lastGame =
                lastGameRaw?.let {
                    RaLastGame(
                            ...
                            imageIcon =
                                    firstString(it, "ImageIcon", "imageIcon")
                                            ?.let(::normalizeImageUrl)
                    )
                }
    ...
    private fun parseRecentlyPlayed(summary: JSONObject): List<RaRecentlyPlayed> {
        val raw =
                summary.optJSONArray("RecentlyPlayed")
                        ?: summary.optJSONArray("recentlyPlayed") ?: return emptyList()
        ...
        for (i in 0 until raw.length()) {
            ...
            out.add(
                    RaRecentlyPlayed(
                            ...
                            imageIcon =
                                    firstString(g, "ImageIcon", "imageIcon")
                                            ?.let(::normalizeImageUrl),
    ...
    private fun parseRecentAchievements(summary: JSONObject): List<RaRecentAchievement> {
        val raw =
                summary.optJSONObject("RecentAchievements")
                        ?: summary.optJSONObject("recentAchievements") ?: return emptyList()
            ...
            while (achKeys.hasNext()) {
                ...
                out.add(
                        RaRecentAchievement(
                                ...
                                badgeUrl =
                                        firstString(ach, "BadgeName", "badgeName")
                                                ?.let(::buildBadgeUrl),
    ...
    private fun parseAwards(awards: JSONObject): RaAwards =
            RaAwards(
                    ...
                    completionAwardsCount =
                            optInt(awards, "CompletionAwardsCount", "completionAwardsCount"),
    ...
    private fun parseCompletion(completion: JSONObject): RaCompletionProgress =
    ...
    private fun firstString(obj: JSONObject, vararg keys: String): String? =
            keys.asSequence()
                    .mapNotNull { key ->
                        obj.opt(key).takeUnless { it == JSONObject.NULL } as? String
                    }
    ...
    private fun RaUserProfile.toJson(): JSONObject =
            JSONObject()
                    ...
                    .put(
                            ...
                            awards?.let {
                                JSONObject()
                                        ...
                                        .put(
                                                "beatenHardcoreAwardsCount",
                                                it.beatenHardcoreAwardsCount
                                        )
                                        .put(
                                                "beatenSoftcoreAwardsCount",
                                                it.beatenSoftcoreAwardsCount
                                        )
                    ...
                    .put(
                            "completionProgress",
                            completionProgress?.let {
                                JSONObject().put("total", it.total).put("count", it.count)
                            }
                    )
    ...
    private fun JSONObject.toProfile(fallbackUsername: String): RaUserProfile? {
        ...
        val recentlyPlayed =
                optJSONArray("recentlyPlayed")?.let { arr ->
                    (0 until arr.length()).mapNotNull { i ->
                        ...
                        RaRecentlyPlayed(
                                ...
                                achievementsTotal =
                                        g.optInt("achievementsTotal").takeIf { it != 0 },
        ...
        val recentAchievements =
                optJSONArray("recentAchievements")?.let { arr ->
                    (0 until arr.length()).mapNotNull { i ->
                        ...
                        RaRecentAchievement(
                                ...
                                hardcore =
                                        if (a.has("hardcore") && !a.isNull("hardcore"))
                                                a.optBoolean("hardcore")
                                        else null
                        )
                    }
                }
                        ?: emptyList()
        val awards =
                optJSONObject("awards")?.let {
                    RaAwards(
                            totalAwardsCount = it.optInt("totalAwardsCount").takeIf { v -> v != 0 },
                            masteryAwardsCount =
                                    it.optInt("masteryAwardsCount").takeIf { v -> v != 0 },
                            completionAwardsCount =
                                    it.optInt("completionAwardsCount").takeIf { v -> v != 0 },
                            beatenHardcoreAwardsCount =
                                    it.optInt("beatenHardcoreAwardsCount").takeIf { v -> v != 0 },
                            beatenSoftcoreAwardsCount =
                                    it.optInt("beatenSoftcoreAwardsCount").takeIf { v -> v != 0 },
                            eventAwardsCount = it.optInt("eventAwardsCount").takeIf { v -> v != 0 },
                            siteAwardsCount = it.optInt("siteAwardsCount").takeIf { v -> v != 0 }
                    )
                }
        val completion =
                optJSONObject("completionProgress")?.let {
                    RaCompletionProgress(
