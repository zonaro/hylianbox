...
/**
 * In-game RetroAchievements overlay: achievement unlock popups, challenge indicator badges and
 * progress readouts, drawn above the emulation surface.
 *
 * Mirrors the Auto-Ocarina HUD integration contract: created by GameActivity, attached last (top
 * z-order), fully non-interactive so touches fall through to the controls beneath. All mutations
...
class RaOverlayView(context: Context) : FrameLayout(context) {
    ...
    /** Popup slot at the top; queued so rapid unlocks don't overwrite each other. */
    private var unlockPopup: LinearLayout? = null
    private var unlockDismissRunnable: Runnable? = null
    private val unlockQueue: ArrayDeque<String> = ArrayDeque()
    ...
    /** Shows the achievement-unlock popup described by [payloadJson]. Queued when busy. */
    fun showUnlock(payloadJson: String) {
        // Validate JSON early off the handler to drop malformed payloads cheaply.
        if (runCatching { JSONObject(payloadJson) }.getOrNull() == null) return
        mainHandler.post {
            if (unlockPopup != null) {
                unlockQueue.addLast(payloadJson)
                return@post
            }
            showUnlockInternal(payloadJson)
        }
    }

    private fun showUnlockInternal(payloadJson: String) {
        val payload =
                runCatching { JSONObject(payloadJson) }.getOrNull()
                        ?: run {
                            showNextQueuedUnlock()
                            return
                        }
        ...
        val badge =
                ImageView(context).apply {
                    ...
                    val url =
                            payload.optString("badge_url").takeIf { it.isNotBlank() }
                                    ?: payload.optString("badge_locked_url").takeIf {
                                        it.isNotBlank()
                                    }
    ...
    private fun showNextQueuedUnlock() {
        val next = unlockQueue.pollFirst() ?: return
        showUnlockInternal(next)
    }
    ...
    fun showChallengeIndicator(payloadJson: String) {
        ...
        mainHandler.post {
            ...
            val badge =
                    ImageView(context).apply {
                        layoutParams = LayoutParams(dp(36), dp(36)).apply { marginEnd = dp(6) }
    ...
    private fun makeMessageRow(): TextView =
            TextView(context).apply {
                setTextColor(ContextCompat.getColor(context, R.color.switch_text_primary))
