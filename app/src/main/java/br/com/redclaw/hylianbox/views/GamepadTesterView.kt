...
/**
 * Live, non-interactive controller visualizer used by [GamepadTesterActivity]. It deliberately
 * observes Android events only: no event is forwarded to a core, and it never changes the frozen
 * touch-control layout.
 */
class GamepadTesterView
@JvmOverloads
constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    fun handleKeyEvent(event: KeyEvent) {
        when (event.action) {
            KeyEvent.ACTION_DOWN -> {
                ...
                N64ControllerMapping.n64ControlForPhysicalKey(event.keyCode, profile, autoTarget)
                        ?.let { control ->
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (mode == Mode.PHYSICAL) drawPhysicalLayout(canvas, inset)
        else drawN64Layout(canvas, inset)
    }
    ...
    private fun drawPhysicalLayout(canvas: Canvas, inset: Float) {
        ...
        drawControl(
                canvas,
                RectF(x + w * .02f, y + h * .01f, x + w * .20f, y + h * .24f),
                label(R.string.tester_lb),
                KeyEvent.KEYCODE_BUTTON_L1
        )
        drawControl(
                canvas,
                RectF(x + w * .22f, y + h * .01f, x + w * .40f, y + h * .24f),
                label(R.string.tester_lt),
                KeyEvent.KEYCODE_BUTTON_L2
        )
        drawControl(
                canvas,
                RectF(x + w * .60f, y + h * .01f, x + w * .78f, y + h * .24f),
                label(R.string.tester_rt),
                KeyEvent.KEYCODE_BUTTON_R2
        )
        drawControl(
                canvas,
                RectF(x + w * .80f, y + h * .01f, x + w * .98f, y + h * .24f),
                label(R.string.tester_rb),
                KeyEvent.KEYCODE_BUTTON_R1
        )
        ...
        drawControl(
                canvas,
                RectF(x + w * .43f, y + h * .28f, x + w * .52f, y + h * .42f),
                label(R.string.tester_select),
                KeyEvent.KEYCODE_BUTTON_SELECT
        )
        drawControl(
                canvas,
                RectF(x + w * .56f, y + h * .28f, x + w * .65f, y + h * .42f),
                label(R.string.tester_start),
                KeyEvent.KEYCODE_BUTTON_START
        )
        ...
        drawControl(
                canvas,
                circle(x + w * .86f, y + h * .42f, faceRadius),
                label(R.string.tester_y),
                KeyEvent.KEYCODE_BUTTON_Y
        )
        drawControl(
                canvas,
                circle(x + w * .80f, y + h * .58f, faceRadius),
                label(R.string.tester_x),
                KeyEvent.KEYCODE_BUTTON_X
        )
        drawControl(
                canvas,
                circle(x + w * .92f, y + h * .58f, faceRadius),
                label(R.string.tester_b),
                KeyEvent.KEYCODE_BUTTON_B
        )
        drawControl(
                canvas,
                circle(x + w * .86f, y + h * .74f, faceRadius),
                label(R.string.tester_a),
                KeyEvent.KEYCODE_BUTTON_A
        )
    }
    ...
    private fun drawN64Layout(canvas: Canvas, inset: Float) {
        ...
        drawN64Control(
                canvas,
                RectF(x + w * .02f, y + h * .01f, x + w * .20f, y + h * .24f),
                label(R.string.tester_n64_l),
                N64ControllerMapping.Control.L,
                n64
        )
        drawN64Control(
                canvas,
                RectF(x + w * .80f, y + h * .01f, x + w * .98f, y + h * .24f),
                label(R.string.tester_n64_r),
                N64ControllerMapping.Control.R,
                n64
        )
        drawN64Control(
                canvas,
                RectF(x + w * .05f, y + h * .28f, x + w * .18f, y + h * .46f),
                label(R.string.tester_n64_z),
                N64ControllerMapping.Control.Z,
                n64
        )
        ...
        drawStick(
                ...
                leftX,
                leftY,
                label(R.string.tester_n64_stick),
                null
        )
        drawN64Control(
                canvas,
                RectF(x + w * .46f, y + h * .88f, x + w * .56f, y + h * .99f),
                label(R.string.tester_n64_start),
                N64ControllerMapping.Control.START,
                n64
        )
        ...
        drawN64Control(
                canvas,
                circle(x + w * .70f, y + h * .40f, cRadius),
                label(R.string.tester_n64_c_up),
                N64ControllerMapping.Control.C_UP,
                n64
        )
        drawN64Control(
                canvas,
                circle(x + w * .64f, y + h * .54f, cRadius),
                label(R.string.tester_n64_c_left),
                N64ControllerMapping.Control.C_LEFT,
                n64
        )
        drawN64Control(
                canvas,
                circle(x + w * .76f, y + h * .54f, cRadius),
                label(R.string.tester_n64_c_right),
                N64ControllerMapping.Control.C_RIGHT,
                n64
        )
        drawN64Control(
                canvas,
                circle(x + w * .70f, y + h * .68f, cRadius),
                label(R.string.tester_n64_c_down),
                N64ControllerMapping.Control.C_DOWN,
                n64
        )
        drawN64Control(
                canvas,
                circle(x + w * .88f, y + h * .52f, cRadius * 1.25f),
                label(R.string.tester_n64_b),
                N64ControllerMapping.Control.B,
                n64
        )
        drawN64Control(
                canvas,
                circle(x + w * .93f, y + h * .72f, cRadius * 1.25f),
                label(R.string.tester_n64_a),
                N64ControllerMapping.Control.A,
                n64
        )
    }
    ...
    private fun drawDpad(canvas: Canvas, cx: Float, cy: Float, radius: Float, physical: Boolean) {
        ...
        if (physical) {
            ...
            drawControl(
                    canvas,
                    positions[1],
                    label(R.string.tester_dpad_down),
                    KeyEvent.KEYCODE_DPAD_DOWN,
                    hatY > AXIS_THRESHOLD
            )
            drawControl(
                    canvas,
                    positions[2],
                    label(R.string.tester_dpad_left),
                    KeyEvent.KEYCODE_DPAD_LEFT,
                    hatX < -AXIS_THRESHOLD
            )
            drawControl(
                    canvas,
                    positions[3],
                    label(R.string.tester_dpad_right),
                    KeyEvent.KEYCODE_DPAD_RIGHT,
                    hatX > AXIS_THRESHOLD
            )
        } else {
            drawN64Control(
                    ...
                    positions[0],
                    label(R.string.tester_dpad_up),
                    N64ControllerMapping.Control.DPAD_UP,
                    ::isN64Pressed,
                    hatY < -AXIS_THRESHOLD
            )
            drawN64Control(
                    ...
                    positions[1],
                    label(R.string.tester_dpad_down),
                    N64ControllerMapping.Control.DPAD_DOWN,
                    ::isN64Pressed,
                    hatY > AXIS_THRESHOLD
            )
            drawN64Control(
                    ...
                    positions[2],
                    label(R.string.tester_dpad_left),
                    N64ControllerMapping.Control.DPAD_LEFT,
                    ::isN64Pressed,
                    hatX < -AXIS_THRESHOLD
            )
            drawN64Control(
                    ...
                    positions[3],
                    label(R.string.tester_dpad_right),
                    N64ControllerMapping.Control.DPAD_RIGHT,
                    ::isN64Pressed,
                    hatX > AXIS_THRESHOLD
            )
        }
    ...
    private fun drawStick(
        ...
        outlinePaint.color =
                if (keyCode != null && isKeyPressed(keyCode)) focusColor else secondaryText
        outlinePaint.strokeWidth = radius * .07f
        canvas.drawCircle(cx, cy, radius, backgroundPaint)
        canvas.drawCircle(cx, cy, radius, outlinePaint)
        backgroundPaint.color = analogColor
        canvas.drawCircle(
                cx + axisX.coerceIn(-1f, 1f) * radius * .55f,
                cy + axisY.coerceIn(-1f, 1f) * radius * .55f,
                radius * .31f,
                backgroundPaint
        )
    ...
    private fun drawControl(
        ...
        drawButton(
                ...
                bounds,
                controlLabel,
                isKeyPressed(keyCode) || extraPressed,
                available,
                amber = false
        )
    }
    ...
    private fun drawN64Control(
            ...
            drawButton(
                    ...
                    bounds,
                    controlLabel,
                    pressedResolver(control) || extraPressed,
                    true,
                    control in C_CONTROLS
            )
    ...
    private fun drawButton(
        ...
        if (isCircle) {
            ...
            canvas.drawCircle(
                    bounds.centerX(),
                    bounds.centerY(),
                    radius - outlinePaint.strokeWidth / 2f,
                    outlinePaint
            )
        } else {
    ...
    private fun isN64Pressed(control: N64ControllerMapping.Control): Boolean {
        if (pressedKeys.any {
                    N64ControllerMapping.n64ControlForPhysicalKey(it, profile, autoTarget) ==
                            control
                }
        )
                return true
        return when (control) {
            ...
            N64ControllerMapping.Control.C_UP ->
                    !N64ControllerMapping.rightStickControlsN64Analog(profile) &&
                            rightY < -AXIS_THRESHOLD
            N64ControllerMapping.Control.C_DOWN ->
                    !N64ControllerMapping.rightStickControlsN64Analog(profile) &&
                            rightY > AXIS_THRESHOLD
            N64ControllerMapping.Control.C_LEFT ->
                    !N64ControllerMapping.rightStickControlsN64Analog(profile) &&
                            rightX < -AXIS_THRESHOLD
            N64ControllerMapping.Control.C_RIGHT ->
                    !N64ControllerMapping.rightStickControlsN64Analog(profile) &&
                            rightX > AXIS_THRESHOLD
            N64ControllerMapping.Control.ANALOG_STICK ->
                    abs(leftX) > AXIS_THRESHOLD ||
                            abs(leftY) > AXIS_THRESHOLD ||
                            (N64ControllerMapping.rightStickControlsN64Analog(profile) &&
                                    (abs(rightX) > AXIS_THRESHOLD || abs(rightY) > AXIS_THRESHOLD))
            else -> false
        }
    }
    ...
    private fun circle(cx: Float, cy: Float, radius: Float) =
