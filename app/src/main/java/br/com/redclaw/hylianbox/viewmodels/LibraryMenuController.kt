package br.com.redclaw.hylianbox.viewmodels
...
/**
 * Host contract implemented by the Library screen so [LibraryMenuController] can trigger
 * Android-side operations (launch, SAF pickers, confirmation dialogs, toasts) without holding an
 * Activity reference itself. Keeping the controller free of direct Android UI ownership makes the
 * menu structure and key handling the only state it owns.
 */
interface LibraryMenuHost {
    /**
     * The host [Activity], used for dialog inflation, resource lookups and gamepad detection. The
     * only implementer ([LibraryActivity]) is an Activity, so the contract is typed as such.
     */
...
/**
 * Owns the per-game context menu shown from the Library grid (via long-press, the tile's overflow
 * button, or physical SELECT/X/Y). Builds the menu with the shared [MenuGridBuilder] so it matches
 * the in-game menu exactly, and mirrors the in-game physical-key handling semantics.
 *
 * The controller holds only menu state (the open dialog, the built sections and badge references);
 * every Android interaction is delegated to [LibraryMenuHost].
 */
class LibraryMenuController(private val host: LibraryMenuHost) {

    private var menuDialog: AlertDialog? = null
    /**
     * Last-built menu sections, kept so physical-key actions reuse the same item action lambdas as
     * tapping a cell (DRY: one code path).
     */
    ...
    private val menuKeyListener =
            View.OnKeyListener { _, keyCode, event -> handleMenuKey(keyCode, event) }

    /** True while the context menu dialog is showing. */
    fun isMenuShowing(): Boolean = menuDialog?.isShowing == true

    /**
     * Open the per-game context menu for [entry]. Mirrors the in-game menu's window sizing: the
     * dialog wraps the content when it fits and scrolls internally (via the root ScrollView) when
     * it would exceed 90% of the screen height.
     */
    ...
    /**
     * Build the menu's category sections and their action items for [entry]. Actions are lambdas
     * bound to the host, so no click-handler switch is needed (DRY: one cell layout, one click
     * path).
     */
                        ...
                        buildList {
                            if (entry.isUserImported) {
                                add(
                                        MenuActionItem(
                                                "change_cover",
                                                R.string.manual_cover_change,
                                                R.drawable.ic_edit_cover
    ...
    /**
     * Run a menu item's action by id, reusing the exact lambda bound in [buildSections] (the same
     * path a cell tap takes). Mirrors the in-game menu's physical-key activation.
     */
    ...
    /**
     * Physical-controller key handler for the context menu, mirroring the in-game
     * [br.com.redclaw.hylianbox.viewmodels.GameActivityViewModel] semantics: act on ACTION_DOWN,
     * consume ACTION_UP (and any other action) so nothing leaks to the grid; B / MODE dismiss;
     * START/RB/LB/Y map to the Start/Export/Import/Uninstall items; A/DPAD_CENTER/ENTER activate
     * the focused cell via its tag. D-pad is left unmapped so focus navigation between cells still
     * works.
     */
     ...
     * without rebuilding the menu. Safe to call when the menu is not showing (the badge list is
     * then empty).
     */
    fun refreshBadges() {
        val context = host.context()
        val show =
                (context as? android.app.Activity)?.let { !GamePad.shouldShowGamePads(it) } ?: false
