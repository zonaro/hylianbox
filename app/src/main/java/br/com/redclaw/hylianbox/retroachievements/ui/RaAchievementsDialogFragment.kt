...
class RaAchievementsDialogFragment : DialogFragment() {
    ...
    private fun loadSingle(hackId: String) {
        ...
        val repository = HylianBoxApp.raCatalogRepository
        showMessage(R.string.ra_loading)

        lifecycleScope.launch {
            val identity =
                    withContext(Dispatchers.IO) {
                        HylianBoxApp.raHashService.ensureIdentity(requireContext(), hackId)
                    }
        ...
        lifecycleScope.launch {
            ...
            val identities =
                    withContext(Dispatchers.IO) {
                        buildMap<
                                String,
                                br.com.redclaw.hylianbox.retroachievements.data.RaGameIdentity> {
