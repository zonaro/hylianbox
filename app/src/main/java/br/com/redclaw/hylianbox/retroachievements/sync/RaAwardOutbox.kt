package br.com.redclaw.hylianbox.retroachievements.sync
...
import br.com.redclaw.hylianbox.retroachievements.api.RaHttpResponse
        ...
        for (award in awards) {
            ...
            if (!journal.acknowledge(award, response)) {
                if (journal.forUser(username).any { it.key == award.key && it.rejected }) {
                    br.com.redclaw.hylianbox.retroachievements.RaNotificationHelper.postSyncFailure(context)
                } else return false
            }
        }
...
class RaAwardSyncWorker(context: Context, parameters: WorkerParameters) : CoroutineWorker(context, parameters) {
    override suspend fun doWork(): Result = try {
        if (HylianBoxApp.raAwardOutbox.flush()) Result.success() else Result.retry()
