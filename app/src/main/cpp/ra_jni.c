                        ...
                        rc_client_server_callback_t callback, void *callback_data, rc_client_t *client)
{
    ...
    if (env != NULL && g_listener != NULL && g_mid_on_server_request != NULL)
    {
        ...
        if (post_data != NULL)
            (*env)->DeleteLocalRef(env, post_data);
    }
    else
    {
...
static void client_event_handler(const rc_client_event_t *event, rc_client_t *client)
{
    ...
    case RC_CLIENT_EVENT_SERVER_ERROR:
    {
        ...
        strbuf_puts(&payload, ",\"api\":");
        strbuf_put_json_string(&payload, err && err->api ? err->api : "");
        strbuf_puts(&payload, "}");
        break;
    }
    ...
    {
        const rc_client_leaderboard_tracker_t *trk = event->leaderboard_tracker;
        strbuf_puts(&payload, "{\"tracker_value\":");
        strbuf_put_json_string(&payload, trk ? trk->display : "");
        strbuf_puts(&payload, ",\"id\":");
        strbuf_put_json_int(&payload, trk ? (long)trk->id : 0);
        strbuf_puts(&payload, "}");
        break;
    }
    case RC_CLIENT_EVENT_LEADERBOARD_SCOREBOARD:
    {
        ...
        strbuf_puts(&payload, "{\"submitted_score\":");
        strbuf_put_json_string(&payload, sb ? sb->submitted_score : "");
        strbuf_puts(&payload, ",\"best_score\":");
        strbuf_put_json_string(&payload, sb ? sb->best_score : "");
        strbuf_puts(&payload, ",\"new_rank\":");
        strbuf_put_json_int(&payload, sb ? (long)sb->new_rank : 0);
        strbuf_puts(&payload, ",\"id\":");
        strbuf_put_json_int(&payload, sb ? (long)sb->leaderboard_id : 0);
        strbuf_puts(&payload, "}");
        break;
    }
    ...
    case RC_CLIENT_EVENT_SUBSET_COMPLETED:
    {
        ...
        strbuf_puts(&payload, "{\"title\":");
        strbuf_put_json_string(&payload, subset ? subset->title : "");
        strbuf_puts(&payload, "}");
        break;
    }
    ...
    JNIEnv *env, jobject thiz)
{
    ...
    if (g_client != NULL)
    {
        len = rc_client_get_user_agent_clause(g_client, buf, sizeof(buf));
    }
    pthread_mutex_unlock(&g_client_mutex);
    if (len == 0)
    {
        snprintf(buf, sizeof(buf), "rcheevos/%s", RCHEEVOS_VERSION_STRING);
    }
    ...
    JNIEnv *env, jobject thiz, jstring response_body)
{
    ...
    jlong game_id = (rc == RC_OK && api_response.response.succeeded)
                        ? (jlong)api_response.game_id
                        : 0;
...
static void append_achievement_json(strbuf_t *sb, const rc_api_achievement_definition_t *a)
{
    ...
    strbuf_puts(sb, ",\"rarity\":");
    {
        char buf[32];
        snprintf(buf, sizeof(buf), "%.4f", (double)a->rarity);
        strbuf_puts(sb, buf);
    }
    strbuf_puts(sb, ",\"rarity_hardcore\":");
    {
        char buf[32];
        snprintf(buf, sizeof(buf), "%.4f", (double)a->rarity_hardcore);
        strbuf_puts(sb, buf);
    }
    strbuf_puts(sb, "}");
}
